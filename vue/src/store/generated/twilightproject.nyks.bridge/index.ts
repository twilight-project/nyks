...
import { EventBroadcastRefund } from "twilight-project-nyks-client-ts/twilightproject.nyks.bridge/types"
...
export { EventRegisterBtcDepositAddress, EventRegisterReserveAddress, EventRegisterJudgeAddress, EventWithdrawBtcRequest, EventSignRefund, EventSignSweep, EventBroadcastRefund, EventProposeRefundHash, Params };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						EventBroadcastRefund: getStructure(EventBroadcastRefund.fromPartial({})),
...
export default {
	...
	getters: {
			...
			return state.SignSweepAll[JSON.stringify(params)] ?? {}
				...
				getBroadcastRefundAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.BroadcastRefundAll[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		...
		async QueryBroadcastRefundAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.TwilightprojectNyksBridge.query.queryBroadcastRefundAll()).data
				
					
				commit('QUERY', { query: 'BroadcastRefundAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryBroadcastRefundAll', payload: { options: { all }, params: {...key},query }})
				return getters['getBroadcastRefundAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryBroadcastRefundAll API Node Unavailable. Could not perform query: ' + e.message)
		...
		async sendMsgConfirmWithdraw({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.TwilightprojectNyksBridge.tx.sendMsgConfirmWithdraw({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgConfirmWithdraw:Init Could not initialize signing client. Wallet is required.')
				}else{
					throw new Error('TxClient:MsgConfirmWithdraw:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
		...
		async MsgConfirmWithdraw({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.TwilightprojectNyksBridge.tx.msgConfirmWithdraw({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgConfirmWithdraw:Init Could not initialize signing client. Wallet is required.')
				} else{
					throw new Error('TxClient:MsgConfirmWithdraw:Create Could not create message: ' + e.message)
				}
			}
		},
