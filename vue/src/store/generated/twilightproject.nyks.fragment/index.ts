...
import { Fragment } from "twilight-project-nyks-client-ts/twilightproject.nyks.fragment/types"
import { fragmentSigners } from "twilight-project-nyks-client-ts/twilightproject.nyks.fragment/types"


export { Params, Fragment, fragmentSigners };
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: twilightproject.nyks.fragment initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.TwilightprojectNyksFragment.query.queryParams()).data
		...
		async sendMsgAcceptSigners({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.TwilightprojectNyksFragment.tx.sendMsgAcceptSigners({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgAcceptSigners({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.TwilightprojectNyksFragment.tx.msgAcceptSigners({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgAcceptSigners:Init Could not initialize signing client. Wallet is required.')
