...
class TwilightAPI {
  ...
  async decodeTransaction(txByteCode) {
    ...
    const endpoints = [
      '/api/decode/decode-transaction', // Local proxy
      `${this.decodeUrl}/decode-transaction`, // Direct (may fail due to CORS)
    ];
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        console.warn(`Decode attempt failed for ${endpoint}:`, error.message);
        continue;
      }
    }

    console.error('All decode attempts failed');
    return null;
  }
