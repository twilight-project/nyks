...
function TransactionDetail() {
  ...
  const [rawBytecode, setRawBytecode] = useState(null);
    ...
    try {
      ...
      for (const msg of txData.messages) {
        if (msg.type.module === 'zkos') {
          ...
          // Get the raw bytecode for display/decoding
          const bytecode = msg.data.txByteCode || msg.data.tx_byte_code;
          if (bytecode) {
            setRawBytecode(bytecode);
            ...
            try {
              ...
              console.warn('Failed to decode transaction:', decodeErr);
            } finally {
  ...
  return (
    <div className="explorer-container">
      ...
      <main className="explorer-main">
        ...
        {/* ZkOS Decoded Transaction */}
        {isZkOS && (rawBytecode || decodedTx) && (
          <>
            {decodeLoading ? (
              <div className="explorer-card">
                <div className="explorer-card-body" style={{ textAlign: 'center', padding: '2rem' }}>
                  <div className="loading-spinner" style={{ margin: '0 auto 1rem' }} />
                  <span style={{ color: 'var(--text-secondary)' }}>Decoding transaction...</span>
                </div>
              </div>
            ) : (
              <DecodedTransaction decoded={decodedTx} rawBytecode={rawBytecode} />
            )}
          </>
        )}

        {/* Fallback: Simple Inputs & Outputs if no decoded data but have zkosData */}
        {isZkOS && zkosData && zkosData.inputs?.length > 0 && !decodedTx && !decodeLoading && (
          <InputsOutputs
